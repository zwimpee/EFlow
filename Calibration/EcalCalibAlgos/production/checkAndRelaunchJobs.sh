...
#    rm -rf ${task}/res/lumiSummary.json
	...
	jobsSubmittingCreated=`cat crab_${task}_status.log | egrep -i "(Submit|Created|Running|Run|Ready)" `
...
#	    fi
	...
	jobsDoneOK=`cat crab_${task}_status.log | egrep -i "(Done|Retrieved)" | awk '{printf "%d,",$1}' | sed -e 's%,$%\n%g'`
	if [ "${jobsDoneOK}AAA" != "AAA" ]; then
	    echo ">>>> Jobs ${jobsDoneOK} are OK for task $task"
	fi
	jobsDoneDup=`cat crab_${task}_status.log | grep -i -A 2 "Wrapper Exit Code" | grep -A 2 "Wrapper Exit Code : 60303" | grep -A2 ">>>>>>>>>" | grep "List of jobs:" | sed -e "s%.*List of jobs: %%g" | awk '{printf "%s,",$1}' | sed -e 's%,$%%g' | awk -f expandJobsList.awk | sed -e 's%,$%%g' ` 
	...
	jobsCrashed=`cat crab_${task}_status.log | grep -i -A 2 "Wrapper Exit Code" | grep -v "Wrapper Exit Code : 0" | grep -v "Wrapper Exit Code : 60303" | grep -A2 ">>>>>>>>>" | grep "List of jobs:" | sed -e "s%.*List of jobs: %%g" | awk '{printf "%s,",$1}' | sed -e 's%,$%%g'` 
	if [ "${jobsCrashed}AAA" != "AAA" ]; then
	    echo ">>>> Relaunching crashed jobs for task $task: ${jobsCrashed}"
	    crab -getoutput ${jobsCrashed} -c $crabdir > crab_${task}_getoutput.log ; sleep 10; crab -forceResubmit ${jobsCrashed} --GRID.ce_black_list='ce02.lcg.cscs.ch,ce11.lcg.cscs.ch' --GRID.se_black_list=${BLACKLIST}  -c $crabdir > crab_${task}_resubmit_crashed.log 2>&1 &
	fi
	
	jobsAborted=`cat crab_${task}_status.log | grep -i -A 2 "Aborted" |  grep -A2 ">>>>>>>>>" | grep "List of jobs:" | sed -e "s%.*List of jobs: %%g" | awk '{printf "%s,",$1}' | sed -e 's%,$%%g'` 
	if [ "${jobsAborted}AAA" != "AAA" ]; then
	    echo ">>>> Relaunching aborted jobs for task $task: jobs ${jobsAborted}"
	    crab -forceResubmit ${jobsAborted} --GRID.ce_black_list='ce02.lcg.cscs.ch,ce11.lcg.cscs.ch' --GRID.se_black_list=${BLACKLIST} -c $crabdir > crab_${task}_resubmit_aborted.log 2>&1 &
	fi
	...
	if [ "${jobsDone}AAA" != ",AAA," ]; then
	    ...
	    for job in `echo ${jobsDone} | sed -e 's%,% %g'`
	      do
	      FILEOK=`eos ls ${CASTORDIR}/${task} | grep  variablesTree_${job} | awk '{print $9}'`
		...
		if [ "${jobsCrashed}AAA" == "AAA"  -a "${jobsAborted}AAA" == "AAA" -a ${nTotalJobs} == ${nJobsDone} ]; then
		    ...
		    crab -getoutput -c $crabdir > crab_${task}_getoutput.log ; sleep 2; crab -report -c $crabdir > crab_${task}_report.log 2>&1 &
