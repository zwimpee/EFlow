...
for item in CFGLIST:
    ...
    if "dbs_pds" in item[0]:
        DBS_PDS_ALL=item[1]
    if "maxtime" in item[0]:
        MAXENDTIME=item[1]
DBS_PDS=DBS_PDS_ALL.split(",")

if (MAXENDTIME != ''):
    MAXTIME=datetime.strptime(str(MAXENDTIME),"%Y-%m-%dT%H:%M:%S")
else:
    MAXTIME=datetime(2099,01,01)
...
#
#sel_runtable="{groupName} ='"+GROUP+"' and {runNumber} >= "+RUNMIN+" and {runNumber} <= "+RUNMAX+" and {datasetName} LIKE '%Online%'"
# the lumisection selection is on the Express dataset:
sel_dstable="{groupName} ='"+GROUP+"' and {runNumber} >= "+RUNMIN+" and {runNumber} <= "+RUNMAX+" and {bfield}>"+BFIELD+" and {datasetName} LIKE '%Online%'"
sel_runtable="{groupName} ='"+GROUP+"' and {runNumber} >= "+RUNMIN+" and {runNumber} <= "+RUNMAX+" and {bfield}>"+BFIELD+" and {datasetName} LIKE '%Online%'"
...
#    
    ...
    if run.isdigit():
        endtime=datetime.strptime(str(line.split(',')[2]),"\"%Y-%m-%dT%H:%M:%S.0\"")
        if (endtime > MAXTIME):
            continue
